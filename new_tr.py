...
class Transformer(nn.Module):
    def __init__(self, vocab_size, num_layers, d_model, max_seq_len, head_num, dropout, train=True, alpha=0.6,
        ...
        # nn.init.constant_(self.weight_matrix.bias, 0)
        #
        self.alpha = alpha
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length, dec_padding_mask, dec_padding_mask2):
        ...
        # timing_signal = tf.cast(timing_signal, tf.float32)
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            with torch.no_grad():
                ...
                # target, vis, target_mask, vis_mask)
                # print('dec_out')
                # print(tf.shape(dec_output))
    ...
    def predict(self, vis, dec_padding_mask,seg_=None):
        ...
        params encoder_outputs: self.encoder()
        params look_ahead_mask: vis_mask
        params dec_padding_mask: target_mask
        """
        # encoder_outputs = tf.cast(encoder_outputs, tf.float32)
        with torch.no_grad():
            batch_size = vis.shape[0]
            # input_length = torch.size(encoder_outputs)[1]

            max_decode_length = self.max_seq_len
            # encoder_decoder_attention_bias = torch.cast(encoder_decoder_attention_bias,tf.float32)
            ...
            # self.encoder(vis, vis_mask, segment, B, T, pre_enc)
            # encoder_outputs = self.encoder(enc_inputs, None, seg_, B)
            encoder_outputs = self.encoder(vis, None, 0, True)
            # print(encoder_outputs.shape)
            cache["encoder_outputs"] = encoder_outputs
            # print(cache["encoder_outputs"].shape)
            # cache["encoder_decoder_attention_bias"] = encoder_decoder_attention_bias
            # make_beam_challenge = make_beam_challenge.cuda()
